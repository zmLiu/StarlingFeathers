package dragonBones.objects
{
	...
	*/
	...
	public class XMLDataParser
	{
		 ...
		 */
		private static var _parseScale:Number;
		public static function parseSkeletonData(skeletonXML:XML,parseScale:Number):SkeletonData
		{
			checkSkeletonXMLVersion(skeletonXML);
			
			_parseScale = parseScale;
		...
		private static function parseDisplayData(displayXML:XML, displayData:DisplayData):void
		{
			...
			displayData.pivotX = Number(displayXML.attribute(ConstValues.A_PIVOT_X)) / _parseScale;
			displayData.pivotY = Number(displayXML.attribute(ConstValues.A_PIVOT_Y)) / _parseScale;
		}
		...
		{
			node.x = Number(xml.attribute(ConstValues.A_X)) / _parseScale;
			node.y = Number(xml.attribute(ConstValues.A_Y)) / _parseScale;
			...
			node.pivotX =  Number(xml.attribute(ConstValues.A_PIVOT_X)) / _parseScale;
			node.pivotY =  Number(xml.attribute(ConstValues.A_PIVOT_Y)) / _parseScale;
